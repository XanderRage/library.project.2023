...
//show to david and ask to explain the syntax
//count[genre]...
 function getMostCommonGenres(books){
  ...
  function getMostPopularBooks(books) {
    const borrowedCount = books.reduce((acc, book) => {
      const { title, borrows } = book;
      const borrowCount = borrows.length;
  
      acc.push({ name: title, count: borrowCount });
 ...
 function getTotalNumberOfBorrows(account, books) {
  ...
  function getTotalNumberOfBorrows(account, books) {
    ...
    const totalBorrows = books.reduce((acc, book) => {
      const borrowCount = book.borrows.filter((borrow) => borrow.id === account).length;
